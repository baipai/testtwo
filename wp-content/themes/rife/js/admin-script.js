...
(function($){
    ...
    window.A13FRAMEWORK_ADMIN = {
		...
		demoDataImporter : function(){
			...
			if(demo_grid.length){
                var status                 = $('#demo_data_import_status'),
                    log_div_container      = $('#demo_data_import_log'),
					...
					startImport = function(e){
                        ...
                        if( import_inputs.filter('[name="clear_content"]').is(':checked') ){
                            confirm_text = confirm_text + "\n" + $(this).data('confirm-remove-content')
                        }

						if (window.confirm(confirm_text)){
                            //setup demo ID that we are interested in
                            ...
                            //scroll to top where info is showing up while importing
                            $('html, body').animate({
                                scrollTop: $("#demo_import_wrapper_anchor").offset().top
                            }, 500);
					...
					nextLevel = function(level, sublevel){
						...
						$.ajax({
                            ...
                            error: function(  jqXHR,  textStatus,  errorThrown ){
                                ...
                                //try to recover from error 404 and 403
                                if( error_count < 10 && typeof jqXHR.status !=='undefined' && (jqXHR.status == 404 || jqXHR.status == 403) ){
                                    //count this error
                                    error_count++;

                                    //log it
                                    log_msg('Error in a row: '+error_count + ' Error status: '+jqXHR.status+'<br />\r\n' );
                                ...
                                if(textStatus == 'parsererror'){
                                    message = 'Importer returned data in wrong format, so it can not continue. Probably unexpected HTML leaked into server reply instead of JSON format.';
                                }
                                else if( errorThrown == 'Internal Server Error' ){
                                    message = 'Server returned "Internal Server Error" while importing, so importer can not continue.';
                                }
                                else{
                ...
                $('.demo_grid_item').each(function(){
                    $(this).tooltipster({
                        content:$(this).find('.hover_content').html(),
                        contentAsHTML: true,
                        interactive: true,
                        arrow: false,
                        position:'right',
                        ...
                        functionReady:function(origin, tooltip){
                            tooltip.on('click','a.demo_import',startImport);
                        }
                        //autoClose:false
                    });
                });
				...
				log_link.on('click',switchLogDiv);
                demo_filter_categories.on('click','li',function(){
                    var filter = $(this).attr('data-filter');

                    demo_filter_categories.find('li').removeClass('active');
                    $(this).addClass('active');

                    if( filter == "*" ){
                        demo_grid.isotope({ filter: '*' });
                    }else{
                        demo_grid.isotope({ filter: '[data-main_category*="'+filter+'"]' });
                    }
                });
                $('.demo_search').on('keyup',debounce(function(){
                    var str = $('.demo_search').val();

                    if( str != '' && str.length > 2 ){
                        demo_grid.isotope({ filter: '[data-categories*="'+str.toLowerCase()+'"]'});
                    }
                    if( str == '' ){
                        demo_grid.isotope({ filter: '*' });
                    }
            ...
            var purchase_code_container = $("#demo_import_wrapper_anchor");

            if(purchase_code_container.length){
                var submit              = purchase_code_container.find('span.code_submit'),
                        ...
                        $.ajax({
                            ...
                            .done(function (data) {
                                ...
                                if (data.response == 'success') {
                                    //$('div.license_code_info').fadeOut(300, function(){ $(this).remove(); });
                                    //$('div.a13_trial_mode_on').fadeOut(300, function(){ $(this).remove(); });
                ...
                submit.on( 'click touchend', function(){
                    var purchase_code = purchase_code_container.find('input[name="add_license_code"]').val();
...
//                                    current_input.focus();
                        ...
                        fillInput = function(){
                            ...
                            current_input.val(prefix + $(this).attr('title'));
                        };
        ...
        settingsAction : function(){
			...
			$('#vc_ui-panel-edit-element').on('click','.a13_delete_selected_nava', function(){
                ...
                if(confirm(G.messages.confirm_delete_nava)){
                    $.ajax({
                        ...
                        data: {
                            action: 'apollo13framework_delete_post',
                            id: id
                        },
            ...
            .on('keyup','.a13_new_nava_id', function(e){
                ...
                $.ajax({
                    ...
                    url: ApolloParams.ajaxurl,
                    data: {
                        action: 'apollo13framework_add_post',
                        title: name
                    },
