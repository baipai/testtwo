...
(function( $ ) {
            ...
            function() {
                ...
                el.each(
                    function() {
                        ...
                        //Apollo13MOD
                        $( this ).find( '.typography-font-multi-style_checkbox_container input' ).on(
                            'change', function() {
                                var tmp_arr = [];
                                ...
                                $(this).closest('.typography-multi-style').find('.typography-font-multi-style').val(JSON.stringify(tmp_arr));
    ...
    redux.field_objects.typography.select = function( selector, skipCheck ) {
        ...
        //Apollo13MOD
        var multi_style = [];
        if( $( '#' + mainID + ' .typography-font-multi-style').length ){
            var multi_style = JSON.parse($( '#' + mainID + ' .typography-font-multi-style' ).val());
        }
        //Apollo13MOD END
        ...
        if ( $( selector ).hasClass( 'redux-typography-family' ) ) {
            ...
            if ( google === true ) {
                ...
                $.each(
                    details.variants, function( index, variant ) {
                        var unique_id = '';
                        if ( $.inArray( variant.id, multi_style) !== -1 || redux.field_objects.typography.size( details.variants ) === 1 ) {
                            checked = ' checked="checked"';
                            style = variant.id;
                        } else {
                            checked = "";
                        }
                        unique_id = Math.random(1).toString().substr(2, 8) + '_' + variant.id;

                        html += '<label for="' + unique_id + '"><input type="checkbox" id="' + unique_id + '" value="' + variant.id + '"' + checked + '/>'+ variant.name.replace(/\+/g, " ") +'</label>';
                    }
                );
                // Instert new HTML
                $( '#' + mainID + ' .typography-font-multi-style_checkbox_container' ).html( html );
                //run again event bind - don't know why, need to check
                $( '#' + mainID + ' .typography-font-multi-style_checkbox_container input' ).on(
                    'change', function() {
                        var tmp_arr = [];
                        $(this).parent().parent().find('input').each( function(){
                            if( $(this).prop('checked') == true ){
                ...
                $( '#' + mainID + ' .typography-font-multi-style_checkbox_container' ).find('input').each( function(){
                    if( $(this).prop('checked') == true ){
                        tmp_arr.push( $(this).val() );
                    }
                } );
                $( '#' + mainID + ' .typography-font-multi-style' ).val(JSON.stringify(tmp_arr));
                //Apollo13MOD END
